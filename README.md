# ERC721 :monkey_face:
A minimalistic implementation of the ERC721 token (A.K.A. NFT) with URI storage using OpenZeppelin library.

It simply mints the NFT and links the URI.

## Prerequisites
You must have [truffle CLI](https://trufflesuite.com/) installed in your computer. Run `truffle version` to verify.

## Scripts

### `truffle compile`
Compiles smart contracts using solidity compiler.

### `truffle migrate [--reset]`
Migrates smart contract code to create ABI files.

### `npm run test`
Runs the suit of tests over the contracts, see `/test` folder to inspect implementation

See more available truffle scripts [here](https://trufflesuite.com/docs/truffle/reference/truffle-commands/).
